...
namespace TadaLib
{
    ...
    public class TadaRigidbody : MonoBehaviour
    {
        ...
        private void Move()
        {
            ...
            {
                // 左端，中央，右端の順に確かめる
                Vector2 origin_left = origin + (Vector2)(Quaternion.Euler(0.0f, 0.0f, transform.eulerAngles.z) * new Vector2(-half_size.x * 0.8f, 0f));
                Vector2 origin_right = origin + (Vector2)(Quaternion.Euler(0.0f, 0.0f, transform.eulerAngles.z) * new Vector2(half_size.x * 0.8f, 0f));
            ...
            // 次に左右
            {
                // まずはx軸方向
                {
                    RaycastHit2D hit_left = Physics2D.BoxCast(origin, new Vector2(half_size.x, half_size.y * 0.6f), 0f, Quaternion.Euler(0.0f, 0.0f, transform.eulerAngles.z) * Vector2.left,
                ...
                {
                    RaycastHit2D hit_right = Physics2D.BoxCast(origin, new Vector2(half_size.x, half_size.y * 0.6f), 0f, Quaternion.Euler(0.0f, 0.0f, transform.eulerAngles.z) * Vector2.right,
                        d.x + half_size.x / 2f, mask_1);
                    Debug.DrawLine(origin, origin + new Vector2(half_size.x + d.x, 0f), Color.blue);
                    if (hit_right)
                    {
